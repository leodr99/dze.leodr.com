...
private ["_nrTowers","_rng","_nrPl","_lCol","_lbrt","_lamb","_twrCl","_stHr","_fnHr","_ndGen","_nrGen","_rngGen","_genCls","_doLit","_gnCnt","_startLogged"];
//Can edit these values
_startLogged = false;
_rng = 2000; //Distance from survivor to detect towers.
_lCol = [1, 0.88, 0.73]; // Colour of lights on tower when directly looked at | RGB Value from 0 to 1.
_lbrt = 0.2;//Brightness (also visible distance) of light source.
_lamb = [1, 0.88, 0.73]; // Colour of surrounding (ambient) light | RGB Value from 0 to 1.
_twrCl = "Land_Ind_IlluminantTower";
_stHr = 18;//Hour (in 24 hours) to start lighting up tower
...
do
{
	...
	if (!(_startLogged)) then {
		_startLogged = true;
		diag_log "#################### Towerlight functions loading complete!";
	};
};