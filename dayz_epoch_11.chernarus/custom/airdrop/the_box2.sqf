...
_message = format["HALV\nCarePackage called in by:\n%1\nTransporter:\n%3\n(%2)\nDistance:\n%4KM" , dayz_playerName,_TheModel,_txt1,(round(_dist/1000))];
...
diag_log format["Halfcare V2.0 Called In by: (%3) - (UID: %4) Start at: %7 Drop at: %1 Model: (%6) DropBoxtype: (%2) WeaponsBox: (%5)", _pos, _randcont, dayz_playerName, _hcGPUID, _randombox,_TheModel,_plane_start]; 
...
while {(_Drop_plane distance _wp_pos) > 500} do {
	...
	if (_hasRC) then {
		if (_DROP_Action==0) then {
			The_DropAction2 = player addAction [
			"<t color='#FF0000'>Dump airdrop cargo ...</t>",
			_thePath+"drop_now2.sqf",
			"",
			5,
			true,
			true,
		...
		<t size='1'font='Bitstream'align='Center'>HALV Supplydrop Enroute @</t><br/>
...
diag_log format["Halfcare V2.0 (HALV) Called In: %1 DropBoxtype (%2) WeaponsBox (%3) %4 dumping package and flying to %5", getPos _Drop_plane, _randcont, _randombox,_TheModel,_plane_end]; 
...
switch (true) do {
	...
	case alive _box: {
		...
		{_bam addWeaponCargoGlobal [_x, 2];} forEach _HALVw2List;
		{_bam addmagazineCargoGlobal [_x, 20];} forEach _HALVm20List;
		{_bam addmagazineCargoGlobal [_x, 10];} forEach _HALVm10List;
		{_bam addmagazineCargoGlobal [_x, 5];} forEach _HALVm5List;
		{_bam addmagazineCargoGlobal [_x, 2];} forEach _HALVm2List;
		...
		if (_randombox isKindOf "MedBox0") then {
			{_bam addmagazineCargoGlobal [_x, 1];} forEach _SesiM01;
